async function renderDouble(sd, stateNames, yrs, activeYear) {
  ...
  const yearData = data.filter(function(point) { return point.YEAR === activeYear})
  let state1 = yearData.filter(function(point) {return point.STATE === stateNames[0].toUpperCase();})
  let state2 = yearData.filter(function(point) { return point.STATE === stateNames[1].toUpperCase();})
